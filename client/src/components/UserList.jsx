  ...
  return (
    <>
        ...
        <div className='md:flex md:flex-wrap md:flex-row flex flex-col animate__animated justify-center items-center animate__fadeIn m-2 w-full'>
            {users.map((user, key) => (
            <Link key={key} to={`/user/${user._id}-${user.username}`} className='flex truncate bg-[#f0f0f0] p-2 md:w-1/3 w-1/2 m-2 rounded cursor-pointer hover:bg-[#2a9d8f] hover:text-white'>
